...
namespace mongo {
    ...
    MONGO_CLIENT_API int getGtLtOp(const BSONElement& e);
    ...
    class LexNumCmp;
    MONGO_CLIENT_API FieldCompareResult compareDottedFieldNames( const std::string& l , 
                                                                 const std::string& r ,
                                                                 const LexNumCmp& cmp );
...
#pragma warning(default: 4275)
     ...
     */
    class MONGO_CLIENT_API BSONSizeTracker {
    public:
        BSONSizeTracker() {
            _pos = 0;
    ...
    // considers order
    MONGO_CLIENT_API bool fieldsMatch(const BSONObj& lhs, const BSONObj& rhs);
}
