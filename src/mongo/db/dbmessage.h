...
#pragma pack(1)
    struct MONGO_CLIENT_API QueryResult : public MsgData {
        long long cursorId;
        int startingFrom;
        int nReturned;
        const char *data() {
...
#pragma pack()
    ...
    */
    class MONGO_CLIENT_API DbMessage {
    public:
        ...
        int pullInt() const { return const_cast<DbMessage *>(this)->pullInt(); }
        int& pullInt() {
            if ( nextjsobj == data )
        ...
        long long pullInt64() const {
            return const_cast<DbMessage *>(this)->pullInt64();
        }
        long long &pullInt64() {
            if ( nextjsobj == data )
                nextjsobj += strlen(data) + 1; // skip namespace
    ...
    class MONGO_CLIENT_API QueryMessage {
    public:
        const char *ns;
        int ntoskip;
        int ntoreturn;
     ...
     */
    struct MONGO_CLIENT_API DbResponse {
        Message *response;
        MSGID responseTo;
    ...
    MONGO_CLIENT_API void replyToQuery(int queryResultFlags,
    ...
    MONGO_CLIENT_API void replyToQuery( int queryResultFlags, Message& m, DbResponse& dbresponse, BSONObj obj );
     ...
     */
    MONGO_CLIENT_API void replyToQuery( int queryResultFlags, Message& response, const BSONObj& resultObj );
} // namespace mongo
