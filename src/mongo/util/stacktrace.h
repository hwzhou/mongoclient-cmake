...
namespace mongo {
    ...
    MONGO_CLIENT_API void printStackTrace(std::ostream &os=log().stream());

#if defined(_WIN32)
    // Print stack trace (using a specified stack context) to "os", default to the log stream.
    MONGO_CLIENT_API void printWindowsStackTrace(CONTEXT &context, std::ostream &os=log().stream());

    // Print error message from C runtime followed by stack trace
    MONGO_CLIENT_API int crtDebugCallback(int, char* originalMessage, int*);
#endif
