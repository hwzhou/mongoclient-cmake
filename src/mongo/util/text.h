...
namespace mongo {
    ...
    class MONGO_CLIENT_API StringSplitter {
    public:
     ...
     */
    MONGO_CLIENT_API bool isValidUTF8(const char *s);
    MONGO_CLIENT_API bool isValidUTF8(const std::string& s);

    // expect that n contains a base ten number and nothing else after it
    // NOTE win version hasn't been tested directly
    MONGO_CLIENT_API long long parseLL( const char *n );

#if defined(_WIN32)

    MONGO_CLIENT_API std::string toUtf8String(const std::wstring& wide);

    MONGO_CLIENT_API std::wstring toWideString(const char *s);

    MONGO_CLIENT_API bool writeUtf8ToWindowsConsole( const char* utf8String, unsigned int utf8StringSize );
...
# endif
    ...
    class MONGO_CLIENT_API WindowsCommandLine {
     ...
     */
    MONGO_CLIENT_API std::string constructUtf8WindowsCommandLine(const std::vector<std::string>& argv);
