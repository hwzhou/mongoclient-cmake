...
namespace mongo {
    ...
    extern MONGO_CLIENT_API const int portSendFlags;
    extern MONGO_CLIENT_API const int portRecvFlags;

    const int SOCK_FAMILY_UNKNOWN_ERROR=13078;

    MONGO_CLIENT_API void disableNagle(int sock);
    ...
    MONGO_CLIENT_API std::string makeUnixSockPath(int port);

    // If an ip address is passed in, just return that.  If a hostname is passed
    // in, look up its ip and return that.  Returns "" on failure.
    MONGO_CLIENT_API std::string hostbyname(const char *hostname);

    MONGO_CLIENT_API void enableIPv6(bool state=true);
    MONGO_CLIENT_API bool IPv6Enabled();
    MONGO_CLIENT_API void setSockTimeouts(int sock, double secs);
    ...
    extern MONGO_CLIENT_API SockAddr unknownAddress; // ( "0.0.0.0", 0 )

    /** this is not cache and does a syscall */
    MONGO_CLIENT_API std::string getHostName();
    
    /** this is cached, so if changes during the process lifetime
     * will be stale */
    MONGO_CLIENT_API std::string getHostNameCached();

    MONGO_CLIENT_API std::string prettyHostName();
